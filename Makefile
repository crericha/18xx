...
prod_deploy : clean
	$(CONTAINER_COMPOSE) run rack rake precompile && \
		rsync --verbose --checksum public/assets/*.js public/assets/*.js.gz public/assets/version.json deploy@gmt:~/18xx/public/assets/ && \
		ssh -l deploy gmt "source ~/.profile && cd ~/18xx/ && git pull && make prod_rack_up_b_d"
